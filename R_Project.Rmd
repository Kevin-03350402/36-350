...
data.df.noredundant$`LC Classification Top Line...2`<- NULL;
data.df.noredundant$ï..Loans..Not.In.House.<-NULL;
...
lengthd = as.data.frame(length)
ggplot(lengthd, aes(x=length)) + 
  geom_histogram(color="red", fill="red",bins=400)+xlim(c(0,1000))+xlab("Loan Duration (Days)")
...
estimate.rate
hist(length,breaks=1000 ,freq = FALSE, xlab="Loan Duration (Days)",
...
starts = as.numeric(strftime(names(loan2018.sorted[1]),format = "%j"))
df10 = as.data.frame(loan2018.sorted)
tmp = df10$Freq
res = ts(tmp,start = c(2018,starts), frequency = 365)
plot(res)

s = spectrum(res, log = "no")


s$freq[which(s$spec>100)]
```
```
The maximum numbers of cycles per unit time are  2.027778 and 52.722222. The wave has 2.027778 peaks and 52.722222. troughs.
```
...
validlcn = suppressWarnings(which(!is.na(x$LC.Classification.Top.Line.1)))


validlc<-x[validlcn,]
s = as.character(validlc$LC.Classification.Top.Line.1)
...
lc = as.character(validlc.upper$LC.Classification.Top.Line.1)
...
title.clean= gsub('[[:punct:] ]+',' ',title)
title.clean1 =gsub("'s","",title.clean)
title.clean2 = unlist(strsplit(title.clean1, split=" "))

title.clean3<- title.clean2[!title.clean2%in%c("")]

result = title.clean3[title.clean3%in%data_stopwords_smart$en==FALSE]

boring = c(as.character(stopwords(language = "en", source = "smart")))

title.clean2= title.clean2[title.clean2!=""]
...
apply(data.df.noredundant,2,FUN =f)
...
info = as.character(validlc.upper$LC.Classification.Top.Line.1)


code = str_extract(info, "[[:digit:]]+")

hist(as.numeric(code),breaks=30)
